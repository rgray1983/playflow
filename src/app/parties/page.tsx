        ...
        <section className="h-full flex-1 overflow-hidden px-6 py-6">
          <header className="mb-5 flex items-start justify-between gap-5">
            ...
            <div className="flex items-center gap-3">
              <Link href="/calendar" className="rounded-[10px] border border-[#B7D4FF] bg-[#EEF5FF] px-4 py-3 text-sm font-semibold text-[#0B55C6]">
                View Calendar
              </Link>
                  ...
                  >
                    <div className="mb-3 flex items-start justify-between gap-3">
                      <div>
                        ...
                        <p className={`mt-1 text-xs ${selectedParty.id === party.id ? "text-white/70" : "text-[#6B7280]"}`}>
                          {party.date} • {party.time}
                        </p>
                      </div>

                      <span className={`rounded-full px-3 py-1 text-xs font-semibold ${selectedParty.id === party.id ? "bg-white/15 text-white" : getStatusStyles(party.status)}`}>
                        {party.status}
                      </span>
                    </div>

                    <div className={`grid grid-cols-2 gap-2 text-xs ${selectedParty.id === party.id ? "text-white/70" : "text-[#6B7280]"}`}>
                      <span>Host: {party.host}</span>
                      <span>Guests: {party.guestCount}</span>
                      <span>Room: {party.room}</span>
            ...
            <section className="overflow-hidden rounded-[12px] border border-black/10 bg-white shadow-sm">
              ...
              <div className="h-full overflow-y-auto p-5 pb-24">
                <div className="mb-4 grid grid-cols-4 gap-3">
                  ...
                  <div className="rounded-[12px] bg-[#F6F0E6] p-4">
                    <p className="text-xs font-semibold text-[#6B7280]">Deposit</p>
                    <p className="mt-2 font-semibold text-[#155E75]">{selectedParty.deposit}</p>
                  </div>
                    ...
                    <div>
                      ...
                      {!detailsExpanded && (
                        <p className="mt-1 text-sm text-[#6B7280]">
                          {selectedParty.host} • Balance {selectedParty.balanceDue} • {selectedParty.addOns.length} add-on{selectedParty.addOns.length === 1 ? "" : "s"}
                        </p>
                      )}
                    </div>
                      ...
                      <div>
                        <div className="grid grid-cols-2 gap-3">
                          ...
                          <div className="rounded-[10px] bg-white p-4">
                            ...
                            <p className="mt-1 text-xs text-[#6B7280]">Birthday Child: {selectedParty.childName}</p>
                          </div>
                        </div>
                      ...
                      <div className="rounded-[10px] bg-white p-4">
                        ...
                        <div className="space-y-2">
                          {selectedParty.addOns.map((addOn) => (
                            <div key={addOn} className="flex items-center justify-between rounded-[8px] bg-[#F6F0E6] px-3 py-3">
                      ...
                      {selectedParty.guests.map((guest) => (
                        <div key={guest.name} className="rounded-[10px] bg-white p-4">
                          <div className="flex items-start justify-between gap-3">
                            ...
                            <span className={`rounded-full px-3 py-1 text-xs font-semibold ${getWaiverStyles(guest.waiver)}`}>
                              Waiver {guest.waiver}
                            </span>
                          </div>
