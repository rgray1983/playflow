...
type EventGuestRecord = { id: string; status: string; rsvpStatus?: string | null; waiverStatus?: string | null };
...
function normalizeEventToParty(event: EventRecord): PartyCard { const guests = event.guests ?? []; const attendingGuests = guests.filter((guest) => guest.rsvpStatus !== "DECLINED"); const startsAt = new Date(event.startTime || event.eventDate); const endsAt = new Date(event.endTime || event.eventDate); return { id: event.id, title: event.title, eventNumber: event.eventNumber || "EVT", eventTypeName: guessEventTypeName(event), startsAt, endsAt, date: formatDate(event.eventDate), time: `${formatTime(event.startTime)} - ${formatTime(event.endTime)}`, status: getStatusLabel(event.status), packageName: event.packageName || "No package", guestOfHonor: event.guestOfHonor || "Guest of Honor", guestCount: Math.max(attendingGuests.length, event.guestOfHonor ? 1 : 0), checkedInCount: attendingGuests.filter((guest) => guest.status === "CHECKED_IN").length, waiverNeededCount: attendingGuests.filter((guest) => guest.waiverStatus !== "SIGNED").length, balanceDue: formatCurrency(event.balanceDue), balanceDueNumber: event.balanceDue }; }
