...
type PartyMode = "before" | "during" | "cleanup" | "after";
...
type Guardrail =
  ...
  | { type: "restore"; party: Party }
  | { type: "status"; party: Party; nextStatus: PartyWorkflowStatus }
  | null;
...
function getStatusLabel(status: string) {
  ...
  if (status === "CLEANING_UP") return "Cleaning Up";
...
function getStatusStyles(status: string) {
  if (status === "Cancelled") return "bg-[#FFE0E9] text-[#9F1239]";
  if (status === "Confirmed" || status === "Ready") return "bg-[#D7F1EC] text-[#155E75]";
  if (status === "Pending") return "bg-[#FFF0C4] text-[#92400E]";
  if (status === "In Progress") return "bg-[#EEF5FF] text-[#0B55C6]";
  if (status === "Cleaning Up") return "bg-[#F6F0E6] text-[#8A6D3B]";
...
function getGuestStatusStyles(status: string) {
  if (status === "Checked In") return "bg-[#D7F1EC] text-[#155E75]";
  if (status === "Checked Out") return "bg-[#EEF5FF] text-[#0B55C6]";
  if (status === "No Show") return "bg-[#F1F1F1] text-[#4B5563]";
...
function normalizeGuestStatus(guest: EventGuestRecord) {
  if (guest.status === "CHECKED_OUT" || guest.checkedOutAt) return "Checked Out";
...
function getPartyMode(status: string): PartyMode {
  ...
  if (status === "CLEANING_UP") return "cleanup";
...
function normalizeEventToParty(event: EventRecord): Party {
  ...
  const guests = realGuests.map((guest) => ({
    id: guest.id,
  ...
  return {
    ...
    expectedCount: realGuests.filter((guest) => guest.status === "EXPECTED").length,
    checkedInCount: realGuests.filter((guest) => guest.status === "CHECKED_IN").length,
    checkedOutCount: realGuests.filter((guest) => guest.status === "CHECKED_OUT").length,
    noShowCount: realGuests.filter((guest) => guest.status === "NO_SHOW").length,
    waiverNeededCount: guests.filter((guest) => guest.waiver !== "Valid").length,
...
export default function PartyManagerPage() {
  ...
  const partyMode = getPartyMode(party.rawStatus);
  const selectedPartyEventType = { color: "#FFB768" };
  const workflowIndex = getWorkflowStepIndex(party.rawStatus);
  const currentWorkflowStep = getWorkflowStep(party.rawStatus);
  const nextWorkflowStep = getNextWorkflowStep(party.rawStatus);
  ...
  const eventReadiness = [
    { label: "Deposit", value: getDepositBadge(party.depositStatus), done: party.depositStatus !== "PENDING", helper: "Deposit record for this booking." },
  ...
  const checklist =
    party.rawStatus === "READY"
      ? [
          { label: "Room is set", done: true },
          { label: "Start party", done: false },
          { label: "Watch guest check-in", done: party.checkedInCount > 0 },
        ]
      : partyMode === "before"
        ? [
            { label: "Review booking details", done: true },
            { label: "Send RSVP link", done: Boolean(party.inviteUrl) },
            { label: "Collect waivers", done: party.waiverNeededCount === 0 },
            { label: "Set up room", done: party.rawStatus === "READY" },
            { label: "Start party", done: false },
          ]
        : partyMode === "during"
          ? [
              { label: "Check in guests", done: party.checkedInCount > 0 },
              { label: "Collect remaining balance", done: party.balanceDueNumber <= 0 },
              { label: "Move to cleaning up", done: false },
            ]
          : partyMode === "cleanup"
            ? [
                { label: "Check out guests", done: party.checkedOutCount > 0 },
                { label: "Final room reset", done: false },
                { label: "Complete party", done: false },
              ]
            : [
  ...
  const primaryAction = party.rawStatus === "CANCELLED" ? "Cancelled" : nextWorkflowStep ? nextWorkflowStep.nextActionLabel : "Completed";
  ...
  function openInviteLink() {
    if (party.inviteUrl) window.open(party.inviteUrl, "_blank", "noopener,noreferrer");
  }

  function emailInviteLink() {
    if (!party.inviteUrl) return;
    const subject = encodeURIComponent(`RSVP link for ${party.title}`);
    const body = encodeURIComponent(`Here is your RSVP + waiver link for ${party.title}:\n\n${party.inviteUrl}\n\nYou can send this to your guests so they can RSVP and sign the waiver before the event.`);
  ...
  async function runPartyAction(currentGuardrail: NonNullable<Guardrail>) {
    const actionKey = currentGuardrail.type === "status" ? `status:${currentGuardrail.nextStatus}` : currentGuardrail.type;
    setActivePartyAction(actionKey);
    setActionError("");

    try {
      const response =
        currentGuardrail.type === "status"
          ? await fetch(`/api/events/${party.id}/status`, {
              method: "POST",
              headers: { "Content-Type": "application/json" },
              body: JSON.stringify({ status: currentGuardrail.nextStatus }),
            })
          : await fetch(`/api/events/${party.id}/${currentGuardrail.type === "restore" ? "uncancel" : "cancel"}`, { method: "POST" });

      const data = await response.json();
      if (!response.ok) throw new Error(data.error || "Unable to update party.");
      await loadParty({ silent: true });
      setGuardrail(null);
    } catch (error) {
      setActionError(error instanceof Error ? error.message : "Unable to update party.");
    } finally {
      setActivePartyAction("");
    }
  }

  function getGuardrailTitle(currentGuardrail: NonNullable<Guardrail>) {
    if (currentGuardrail.type === "cancel") return `Cancel ${currentGuardrail.party.title}?`;
    if (currentGuardrail.type === "restore") return `Restore ${currentGuardrail.party.title}?`;
    return getWorkflowStep(currentGuardrail.nextStatus).confirmationTitle.replace("this party", currentGuardrail.party.title);
  }

  function getGuardrailBody(currentGuardrail: NonNullable<Guardrail>) {
    if (currentGuardrail.type === "cancel") {
      return "This will remove the party from the active dashboard. The booking, guest list, timeline, and payment records will be preserved. You can restore it later from Cancelled parties.";
    }
    if (currentGuardrail.type === "restore") {
      return "This will move the party back into the active workflow so staff can continue managing guests, payments, and timeline activity.";
    }
    return getWorkflowStep(currentGuardrail.nextStatus).confirmationBody;
  }

  function getGuardrailButton(currentGuardrail: NonNullable<Guardrail>) {
    if (currentGuardrail.type === "cancel") return "Cancel Party";
    if (currentGuardrail.type === "restore") return "Restore Party";
    return getWorkflowStep(currentGuardrail.nextStatus).nextActionLabel;
  }
    ...
    <main className="h-screen overflow-hidden bg-[#E7E3DA] p-5 text-[#202633] antialiased">
        ...
        <section className="h-full overflow-hidden px-6 py-6">
          <header className="mb-5 flex items-start justify-between gap-5">
            ...
            <div className="flex items-center gap-3">
              ...
              {party.rawStatus === "CANCELLED" ? (
                <button onClick={() => setGuardrail({ type: "restore", party })} disabled={!party.id || Boolean(activePartyAction)} className="rounded-[10px] border border-[#B7D4FF] bg-[#EEF5FF] px-4 py-3 text-sm font-semibold text-[#0B55C6] disabled:opacity-40">Restore Party</button>
              ) : (
                <button onClick={() => setGuardrail({ type: "cancel", party })} disabled={!party.id || party.rawStatus === "COMPLETED" || Boolean(activePartyAction)} className="rounded-[10px] border border-[#FCA5A5] bg-white px-4 py-3 text-sm font-semibold text-[#9F1239] disabled:opacity-40">Cancel Party</button>
              )}
              <button
                disabled={!party.id || party.rawStatus === "CANCELLED" || !nextWorkflowStep || Boolean(activePartyAction)}
                onClick={() => nextWorkflowStep && setGuardrail({ type: "status", party, nextStatus: nextWorkflowStep.status })}
                className="rounded-[10px] bg-[#1E293B] px-5 py-3 text-sm font-semibold text-white disabled:opacity-40"
              >
                {activePartyAction.startsWith("status") ? "Updating..." : primaryAction}
              </button>
            </div>
          </header>
              ...
              <div className="h-full overflow-y-auto p-5 pb-24">
                ...
                <section className="mb-4 rounded-[14px] border border-black/10 bg-[#F6F0E6] p-4">
                  <div className="mb-4 flex items-start justify-between gap-4">
                    <div>
                      <p className="text-xs font-semibold uppercase tracking-[0.08em] text-[#8A6D3B]">Party Status</p>
                      <h3 className="mt-1 text-xl font-semibold tracking-[-0.03em] text-[#1E293B]">{currentWorkflowStep.label}</h3>
                      <p className="mt-1 max-w-[720px] text-sm text-[#6B7280]">{currentWorkflowStep.description}</p>
                    </div>
                    <div className="text-right">
                      <p className="text-xs font-semibold uppercase tracking-[0.08em] text-[#8A6D3B]">Next Action</p>
                      <p className="mt-1 text-sm font-semibold text-[#1E293B]">{nextWorkflowStep ? nextWorkflowStep.nextActionLabel : "Workflow Complete"}</p>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-6 gap-2">
                    {partyWorkflowSteps.map((step, index) => {
                      ...
                      const isCurrent = step.status === party.rawStatus;
                      return (
                        <div key={step.status} className={`rounded-[10px] border px-3 py-3 ${isCurrent ? "border-[#1E293B] bg-white" : isComplete ? "border-[#D7F1EC] bg-white/80" : "border-black/10 bg-white/50"}`}>
                          <div className={`mb-2 flex h-7 w-7 items-center justify-center rounded-full text-xs font-semibold ${isCurrent ? "bg-[#1E293B] text-white" : isComplete ? "bg-[#D7F1EC] text-[#155E75]" : "bg-[#F6F0E6] text-[#6B7280]"}`}>{isComplete ? "✓" : index + 1}</div>
                          <p className="text-xs font-semibold text-[#1E293B]">{step.shortLabel}</p>
                        </div>
                      );
                    })}
                  </div>
                ...
                <section className="mb-4 rounded-[14px] border border-black/10 bg-[#F6F0E6] p-4">
                  <div className="mb-4 flex items-center justify-between gap-4">
                    <div>
                      <p className="text-xs font-semibold uppercase tracking-[0.08em] text-[#8A6D3B]">Recommended Work</p>
                      <h3 className="mt-1 text-xl font-semibold tracking-[-0.03em] text-[#1E293B]">Next Step: {nextStep.label}</h3>
                      <p className="mt-1 text-sm text-[#6B7280]">There is always one obvious action at the top right to move this party forward.</p>
                    </div>
                ...
                <section className="mb-4 grid grid-cols-5 gap-2 rounded-[14px] border border-black/10 bg-white p-3">
                  {[
                    ...
                    <button key={card.key} onClick={() => setOpenCard(card.key)} className={`rounded-[10px] px-3 py-3 text-left transition ${openCard === card.key ? "bg-[#1E293B] text-white" : "bg-[#F6F0E6] text-[#1E293B] hover:bg-[#EFE8DC]"}`}>
                      <span className="block text-sm font-semibold">{card.label}</span>
                      <span className={`mt-1 block truncate text-xs ${openCard === card.key ? "text-white/70" : "text-[#6B7280]"}`}>{card.detail}</span>
                    </button>
                  ))}
                </section>
                ...
                {openCard === "next" && (
                  <section className="rounded-[12px] border border-black/10 bg-[#F6F0E6] p-4">
                    <h3 className="text-lg font-semibold tracking-[-0.03em] text-[#1E293B]">{party.eventTypeName} Checklist</h3>
                    <div className="mt-4 space-y-2">
                      {checklist.map((item, index) => {
                        const isNext = item.label === nextStep.label;
                        return (
                          <div key={item.label} className={`flex items-center gap-3 rounded-[10px] px-3 py-2.5 ${isNext ? "border border-[#1E293B] bg-white" : item.done ? "bg-white/70" : "bg-white"}`}>
                            <div className={`flex h-7 w-7 shrink-0 items-center justify-center rounded-full text-xs font-semibold ${item.done ? "bg-[#D7F1EC] text-[#155E75]" : isNext ? "bg-[#1E293B] text-white" : "bg-[#F6F0E6] text-[#6B7280]"}`}>{item.done ? "✓" : index + 1}</div>
                            <div className="flex-1"><p className="text-sm font-semibold text-[#1E293B]">{item.label}</p>{isNext && <p className="text-[11px] text-[#6B7280]">Next action</p>}</div>
                          </div>
                        );
                      })}
                    </div>
                ...
                {openCard === "details" && <section className="rounded-[12px] border border-black/10 bg-[#F6F0E6] p-4"><h3 className="text-lg font-semibold tracking-[-0.03em] text-[#1E293B]">Details + Notes</h3><div className="mt-4 grid grid-cols-2 gap-3"><div className="rounded-[10px] bg-white p-4"><p className="text-xs font-semibold text-[#6B7280]">Guest of Honor</p><p className="mt-2 font-semibold text-[#1E293B]">{party.childName}</p></div><div className="rounded-[10px] bg-white p-4"><p className="text-xs font-semibold text-[#6B7280]">Package</p><p className="mt-2 font-semibold text-[#1E293B]">{party.packageName}</p></div><div className="rounded-[10px] bg-white p-4"><p className="text-xs font-semibold text-[#6B7280]">Room</p><p className="mt-2 font-semibold text-[#1E293B]">{party.room}</p></div><div className="rounded-[10px] bg-white p-4"><p className="text-xs font-semibold text-[#6B7280]">Phone</p><p className="mt-2 font-semibold text-[#1E293B]">{party.phone}</p></div></div><div className="mt-3 rounded-[10px] bg-white p-4"><p className="text-xs font-semibold text-[#6B7280]">Notes</p><p className="mt-2 text-sm text-[#1E293B]">{party.notes}</p></div></section>}
              </div>
            </section>

            <aside className="overflow-y-auto rounded-[12px] border border-black/10 bg-white p-5 shadow-sm">
              <h3 className="text-lg font-semibold tracking-[-0.03em] text-[#1E293B]">Quick Actions</h3>
              <div className="mt-4 space-y-2">
                <button disabled={!party.inviteUrl} onClick={copyInviteLink} className="w-full rounded-[10px] bg-[#F6F0E6] px-4 py-3 text-left text-sm font-semibold text-[#1E293B] disabled:opacity-40">Copy RSVP Link {copyStatus && <span className="text-[#155E75]">• {copyStatus}</span>}</button>
                <button disabled={!party.inviteUrl} onClick={emailInviteLink} className="w-full rounded-[10px] bg-[#F6F0E6] px-4 py-3 text-left text-sm font-semibold text-[#1E293B] disabled:opacity-40">Email RSVP Link</button>
                <Link href="/parties" className="block w-full rounded-[10px] bg-[#F6F0E6] px-4 py-3 text-left text-sm font-semibold text-[#1E293B]">Back to Control Center</Link>
              </div>
              <div className="mt-6 rounded-[12px] bg-[#F6F0E6] p-4">
                <p className="text-xs font-semibold uppercase tracking-[0.08em] text-[#8A6D3B]">Workflow Rule</p>
                <p className="mt-2 text-sm text-[#6B7280]">If PlayFlow recommends a next step, the top-right action button performs that step.</p>
              </div>
            </aside>
          </div>
        </section>
      ...
      {guardrail && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
          <div className="w-full max-w-[460px] rounded-[18px] bg-white p-6 shadow-xl">
            <p className="text-xs font-semibold uppercase tracking-[0.08em] text-[#8A6D3B]">Confirm Action</p>
            <h2 className="mt-2 text-2xl font-semibold tracking-[-0.04em] text-[#1E293B]">
              {guardrail.type === "cancel" ? `Cancel ${party.title}?` : guardrail.type === "restore" ? `Restore ${party.title}?` : getWorkflowStep(guardrail.nextStatus).confirmationTitle.replace("this party", party.title)}
            </h2>
            <p className="mt-3 text-sm leading-6 text-[#6B7280]">
              {guardrail.type === "cancel" && "This will remove the party from the active dashboard. The booking, guest list, timeline, and payment records will be preserved. You can restore it later from Cancelled parties."}
              {guardrail.type === "restore" && "This will move the party back into the active workflow so staff can continue managing guests, payments, and timeline activity."}
              {guardrail.type === "status" && getWorkflowStep(guardrail.nextStatus).confirmationBody}
            </p>
            <div className="mt-5 rounded-[12px] bg-[#F6F0E6] p-4 text-sm text-[#1E293B]">
              <p className="font-semibold">{party.title}</p>
              <p className="mt-1 text-[#6B7280]">{party.date} • {party.time}</p>
            </div>
            <div className="mt-6 grid grid-cols-2 gap-3">
              <button onClick={() => setGuardrail(null)} disabled={Boolean(activePartyAction)} className="rounded-[10px] border border-black/10 bg-white px-4 py-3 text-sm font-semibold text-[#1E293B] disabled:opacity-40">Not Yet</button>
              <button onClick={() => runPartyAction(guardrail)} disabled={Boolean(activePartyAction)} className={`rounded-[10px] px-4 py-3 text-sm font-semibold text-white disabled:opacity-40 ${guardrail.type === "cancel" ? "bg-[#9F1239]" : "bg-[#1E293B]"}`}>
                {activePartyAction ? "Working..." : guardrail.type === "cancel" ? "Cancel Party" : guardrail.type === "restore" ? "Restore Party" : getWorkflowStep(guardrail.nextStatus).nextActionLabel}
              </button>
            </div>
          </div>
        </div>
